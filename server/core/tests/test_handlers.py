...
#        self.assertEqual(round(second_a['time']), PinpointHandler.SECONDS)
    ...
    def test_accept_friendship(self):
        ...
        self._logout()
        bob = self._login(username='bob')
        response = self.client.post(url, {'id': str(peter['_id'])})
        self.assertEqual(response.code, 200)
        self.assertTrue(json.loads(response.body)['notified'], True)
        email_sent = mail.outbox[-1]
        self.assertTrue(peter['email'] in email_sent.recipients())
        self.assertTrue('Bob' in email_sent.subject)
        self.assertTrue('Hi Peter' in email_sent.body)
